...
resource "azurerm_key_vault_secret" "iac_backend_public_ssh_key" {
    name         = var.iac_public_ssh_secret_name
    value        = tls_private_key.iac_backend_ssh_key.public_key_openssh
    key_vault_id = azurerm_key_vault.iac_backend.id
}

resource "azurerm_key_vault_secret" "iac_backend_private_ssh_key" {
    name         = var.iac_private_ssh_secret_name
...
su  azureuser -c 'cd ~/myagent && ./config.sh --unattended --url "${var.devops_url}" --auth pat --token "${var.personal_access_token_value}" --pool "${var.agent_pool}" --agent "${var.iac_agent_name}" --work _work --runAsService'
...
resource "azurerm_network_interface" "iac_bcakend_vm" {
    name                = var.iac_backend_vm_nic_name
...
resource "azurerm_linux_virtual_machine" "iac_bcakend_vm" {
    name                = var.iac_backend_vm_name
    location            = data.azurerm_resource_group.aks_backend.location
    resource_group_name = data.azurerm_resource_group.aks_backend.name
    network_interface_ids = [
        azurerm_network_interface.iac_bcakend_vm.id,
    ]
    size                = "Standard_B2s"
    computer_name  = var.iac_backend_vm_name
