...
public class X509Provider : IX509Provider
{
    ...
    {
        using (var store = _x509CertificateWrapper.Open(OpenFlags.ReadOnly, StoreName.My, storeLocation: _authenticationSettings.StoreLocation ?? StoreLocation.LocalMachine))
        {
    ...
    private X509Certificate2 GenerateTemporaryAnonymousCertificate()
    {
        ...
        using (var store = _x509CertificateWrapper.Open(OpenFlags.ReadWrite, StoreName.My, storeLocation: _authenticationSettings.StoreLocation ?? StoreLocation.LocalMachine))
        {
