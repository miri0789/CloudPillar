...
public class RunDiagnosticsHandler : IRunDiagnosticsHandler
{
    ...
    private async Task<string> UploadFileAsync(string diagnosticsFilePath, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var actionToReport = new ActionToReport(TwinPatchChangeSpec.Diagnostics);
    ...
    private async Task<TwinActionReported> GetDownloadStatus(string fileName)
    {
        ...
        var desiredList = twinDesired?.GetDesiredChangeSpecByKey(TwinPatchChangeSpec.Diagnostics)?.Patch?.FirstOrDefault().Value;
        var reportedList = twinReported?.GetReportedChangeSpecByKey(TwinPatchChangeSpec.Diagnostics)?.Patch?.FirstOrDefault().Value;
