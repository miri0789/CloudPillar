...
public class FileDownloadHandler : IFileDownloadHandler
{
    ...
    public bool AddFileDownload(ActionToReport actionToReport)
    {
        ...
        if (fileDownload == null)
        {
            var existFile = _filesDownloads.Any(item =>
                string.IsNullOrWhiteSpace(item.Action.DestinationPath) ||
                _fileStreamerWrapper.GetFullPath(item.Action.DestinationPath) ==
                _fileStreamerWrapper.GetFullPath(((DownloadAction)actionToReport.TwinAction).DestinationPath));
            if (existFile)
            {
                _logger.Warn($"File {((DownloadAction)actionToReport.TwinAction).DestinationPath} is already exist in download list");
                return false;
            }
    ...
    private async Task HandleEndRangeDownloadAsync(string filePath, DownloadBlobChunkMessage message, FileDownload file, CancellationToken cancellationToken)
    {
        ...
        if (!isRangeValid)
        {
            await _d2CMessengerHandler.SendFirmwareUpdateEventAsync(cancellationToken, message.FileName, message.ChangeSpecId, file.ActionReported.ReportIndex, message.RangeIndex.ToString(), message.RangeStartPosition, message.RangeEndPosition);
