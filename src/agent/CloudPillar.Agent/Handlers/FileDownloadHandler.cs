...
public class FileDownloadHandler : IFileDownloadHandler
{
    ...
    public async Task ReloadKnownIdentitiesFFromCertificates(CancellationToken cancellationToken)
    {
        ...
        {
            _logger.Error($"ReloadKnownIdentitiesFFromCertificates failed message: {ex.Message}");
        }
        finally
        {
            _logger.Info($"ReloadKnownIdentitiesFFromCertificates stopped");
            timer.Dispose();
        }
    }
}
