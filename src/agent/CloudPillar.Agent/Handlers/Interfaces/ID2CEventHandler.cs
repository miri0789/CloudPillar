...
public interface ID2CMessengerHandler
{
    ...
    Task SendStreamingUploadChunkEventAsync(byte[] buffer, Uri storageUri, string actionId, long currentPosition, string checkSum, bool fromRunDiagnostic = false);
    Task ProvisionDeviceCertificateEventAsync(X509Certificate2 certificate);
    Task SendDeleteDiagnosticsBlobAsync(Uri storageUri);
}