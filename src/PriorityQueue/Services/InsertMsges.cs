...
public class InsertMsges : BackgroundService
{
    ...
    private async Task InitializeProcessors(List<string> serviceBusUrls)
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            var randomQueueName = serviceBusUrls[2];
             var sender = client.CreateSender(randomQueueName);

            // Create and send a message
            var messageBody = $"Hello, Service Bus! Message p3 loop {i + 1} to {randomQueueName}";
        ...
        {
            // Choose a random queue
            var randomQueueName = serviceBusUrls[1];
             var sender = client.CreateSender(randomQueueName);

            // Create and send a message
            var messageBody = $"Hello, Service Bus! Message p2 loop {i + 1} to {randomQueueName}";
            var message = new ServiceBusMessage(Encoding.UTF8.GetBytes(messageBody));
