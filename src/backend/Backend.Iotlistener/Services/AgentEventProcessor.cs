...
public class AgentEventProcessor : IEventProcessor
{
    ...
    public async Task ProcessEventsAsync(PartitionContext context, IEnumerable<EventData> messages)
    {
        ...
        foreach (var eventData in messages)
        {
            ...
            await HandleMessageAsyncAsync(eventData, !String.IsNullOrWhiteSpace(drainD2cQueues), context.PartitionId);
        }
        await context.CheckpointAsync();
    }

    private async Task HandleMessageAsyncAsync(EventData eventData, bool isDrainMode, string partitionId)
    {
        try
        {
