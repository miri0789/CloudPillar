...
resource "null_resource" "add-namespaces" {
  provisioner "local-exec" {
    command = <<-EOT
        ...
        az aks command invoke -g cp-ms-${var.env}-rg -n "cp-${var.env}-aks" -c 'kubectl create namespace cn-dns'
    EOT
    interpreter = ["PowerShell", "-Command"]
  }
}
...
resource "null_resource" "dns-cred" {
    ...
    command = <<-EOT
        $jsonStr = @'
        {
          ...
          "resourceGroup": "${data.azurerm_resource_group.int-cyber.name}",
        ...
        az aks command invoke -g ${local.aks-pfx}-rg -n ${local.aks-pfx}-aks -c 'kubectl create secret generic dns-update-cred --from-file=azure.json=azure.json -n cn-dns' -f .
    EOT
    interpreter = ["PowerShell", "-Command"]
    working_dir = "${path.module}"
  }
...
resource "null_resource" "helm-repos-updates" {
    provisioner "local-exec" {
    command = <<-EOT
        az aks command invoke -g cp-ms-${var.env}-rg -n cp-${var.env}-aks -f . -c 'helm repo add spv-charts https://charts.spvapi.no && helm upgrade --install akv2k8s spv-charts/akv2k8s --namespace akv2k8s --values values-akv2k8s${contains(["dev","tst"],terraform.workspace)?"-spot":""}.yaml --debug'
        az aks command invoke -g cp-ms-${var.env}-rg -n cp-${var.env}-aks -f . -c 'helm repo add bitnami https://charts.bitnami.com/bitnami && helm upgrade --install externaldns bitnami/external-dns ${contains(["dev","tst"],terraform.workspace)?"--values values-cndns.yaml":""} --set provider=azure --set azure.secretName=dns-update-cred --set azure.secretNamespace=cn-dns --set domainFilters[0]=cn-int-tp-${terraform.workspace}.net --namespace cn-dns'
        az aks command invoke -g cp-ms-${var.env}-rg -n cp-${var.env}-aks -f . -c 'helm repo add traefik https://traefik.github.io/charts && helm upgrade --install traefik traefik/traefik -n traefik --values values-traefik${contains(["dev","tst"],terraform.workspace)?"-spot":""}.yaml --debug'
    EOT
    interpreter = ["PowerShell", "-Command"]
    working_dir = "${path.module}"
  }
...
resource "null_resource" "mesh-ips" {
    ...
    command = <<-EOT
      ...
      az aks command invoke --resource-group cp-ms-${var.env}-rg --name cp-${var.env}-aks -f . --command 'kubectl patch meshconfig osm-mesh-config -n kube-system -o json --type=merge --patch-file patch-file.yaml'
    EOT
    interpreter = ["PowerShell", "-Command"]
    working_dir = "${path.module}"
  }
...
resource "null_resource" "remove-temaple-files" {
  provisioner "local-exec" {
    command = <<-EOT
       ...
       Remove-Item -path values-cndns.yaml
