...
resource "null_resource" "akv-secret" {
    ...
    command = <<-EOT
        ...
        secret=$(az keyvault secret show --name $secret_name --vault-name $vault_name --query value -o tsv 2>/dev/null)
        ...
        (Get-Content AzureKeyVaultSecretTraefik.yaml) -replace "{cert}", "${azurerm_app_service_certificate_order.app-cert.certificates[0].key_vault_secret_name}" | Set-Content -Path 'AzureKeyVaultSecretTraefik.yaml' -encoding utf8
